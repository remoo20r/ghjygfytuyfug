...
class _PlayerScreenState extends ConsumerState<PlayerScreen> {
  ...
  void initState() {
    ...
    // Android TV: keep buffering low for fast live-channel starts, and make
    // audio resilient. We deliberately DO NOT force hwdec/vo here — forcing
    // 'mediacodec-copy' caused a black screen when switching channels (the
    // decoder surface wasn't released before the next channel attached) and
    // stutter/av-desync on some codecs. media_kit's default 'auto-safe' hwdec
    // picks hardware when it works and falls back to software automatically,
    // which is what we want.
    if (Platform.isAndroid && native is NativePlayer) {
      unawaited(() async {
        try {
          // Low-latency live buffering, light on RAM.
          await native.setProperty('cache', 'yes');
          await native.setProperty('demuxer-max-bytes', '12MiB');
          await native.setProperty('demuxer-max-back-bytes', '6MiB');
          // Don't abort the whole stream if one audio frame fails to decode.
